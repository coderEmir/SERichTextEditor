...
- (instancetype)init {
  if (self = [super init]) {
    self.option = [[ASPopoverOption alloc] init];
  ...
  if (self.superview) {
    ...
    [UIView animateWithDuration:_option.animationOut animations:^{
      ...
      if (self.didDismissHandler) {
        ...
        self.contentView.frame = self.cacheFrame;
    }];
  }
}
...
#define selfWidth self.bounds.size.width - 2
...
//        arrowPoint = CGPointMake(arrowPoint.x - 10, arrowPoint.y);
      ...
      ArrowAddLineToPoint(selfCornerRadius, selfHeight - selfArrowHeight);
        ...
        [arrow addArcWithCenter:CGPointMake(selfCornerRadius + 0.5, selfHeight -  selfArrowHeight - selfCornerRadius) radius:selfCornerRadius startAngle:radians(90) endAngle:radians(180) clockwise:YES];
        [arrow addArcWithCenter:CGPointMake(selfCornerRadius + 0.5, selfCornerRadius) radius:selfCornerRadius startAngle:radians(180) endAngle:radians(270) clockwise:YES];
        ...
        CGFloat rigntPadding = selfCornerRadius + arrowPoint.x - CGRectGetMaxX(rect);
        rigntPadding = rigntPadding > 0 ? rigntPadding : selfCornerRadius * 0.5;
        CGFloat endAngle = acos(cos(rigntPadding / selfCornerRadius)) * 180;
    [arrow addArcWithCenter:CGPointMake(selfWidth - selfCornerRadius, selfHeight - selfArrowHeight - selfCornerRadius) radius:selfCornerRadius startAngle:radians(0) endAngle:radians(endAngle) clockwise:YES];
