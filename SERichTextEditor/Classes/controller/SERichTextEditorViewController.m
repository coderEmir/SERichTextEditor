...
- (NSArray *)singleSelectEventInNewBtn:(UIButton *)btn oldBtn:(UIButton *)oldBtn tag:(NSInteger)tag imageNames:(NSArray *)imageNames
{
    ...
    {
        oldBtn.selected = NO;
        NSString *btnImgName = [imageNames[oldBtn.tag - tag] stringByAppendingString:@"@2x.png"];
        [oldBtn setImage:[self imageAtPath:[self fileName:btnImgName]] forState:UIControlStateNormal];
    }
    btn.selected = !btn.selected;
...
- (void)resetButtonImage:(UIButton *)btn styleImageName:(NSString *)styleImageName
{
    ...
    btnImgName = [btnImgName stringByAppendingString:@"@2x.png"];
    [btn setImage:[self imageAtPath:[self fileName:btnImgName]] forState:UIControlStateNormal];
}
...
- (void)resetFont:(NSInteger)index hadFont:(BOOL)hadFont
{
    ...
    NSString *imageName = hadFont ? selectImgName : self.textFontImgNames[index];
    imageName = [imageName stringByAppendingString:@"@2x.png"];
    
    [btn setImage:[self imageAtPath:[self fileName:imageName]] forState:UIControlStateNormal];
...
- (UIView *)fontSizeView
{
    if (!_fontSizeView)
    {
        ...
        [btn setImage:[self imageAtPath:[self fileName:@"fontSizeNomalSelected@2x.png"]] forState:UIControlStateNormal];
        btn.selected = YES;
...
- (void)addSubFunctionButton:(NSArray *)imgNames selector:(SEL)action tag:(NSInteger)tag toView:(UIView *)superView
{
    for (NSInteger i = 0; i < imgNames.count; i ++) {
        ...
        NSString *imageName = [imgNames[i] stringByAppendingString:@"@2x.png"];
        [styleBtn setImage:[self imageAtPath:[self fileName:imageName]] forState:UIControlStateNormal];
        styleBtn.tag = tag + i;
